...
Now, for the actual task above: you MUST explicitly write the exact tool name from the list above (e.g. "I will use the web_search tool to..."). Do not just output a raw answer or explanation without naming which tool executes it. Two more requirements for how you close your answer: (1) If completing this action requires writing or passing a literal value that depends on earlier results in this conversation (for example, the content of a file to create, or a specific computed value): check whether that value is already available, verbatim, in [PLAN_STATE] or in a completed tool result above. If it is available, you MUST state it explicitly and completely in your answer now — do not just describe the intention to use it later. If it is NOT yet available — for example, no prior step has actually gathered the information yet — do NOT invent, guess, or fabricate a plausible-looking value. Instead, propose the tool call needed to obtain that information first, and leave the literal value for a later turn once a completed step has actually produced it. (2) End your answer with a short final sentence naming only the single tool you have decided to use, as the very last words of your response — for example: '...using the write tool.' Do not name any other tool after that point.${antiSelfConditioningNote}${literalFidelityNote}`
